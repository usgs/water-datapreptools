...
#
# Modfied Spring 2015 by Robert Ourso (Alaska Science Center) and Kitty Kolb (SAWSC)
# Updated to create option for states using 30 m data
# Ultimately some day Kitty and/or Bob will further update it to reflect arcpy and not
# gp, but this gets the job done
# Bob's notes:  # Changed the 10m to 30m, rtourso, 03/12/2015, at least that's what it appears to be. Nobody remembers
# Alaska is 30 years behind the Outside, dang it. Maybe that's what the 30 really means. Search for my name to find
# the edits...or don't...O
# ---------------------------------------------------------------------------
...
try:
    ...
    gp.FeatureToRaster_conversion(nhd_area_Layer, "dummy", areatempraster, "30")      
except:
    gp.CreateRasterDataset_management(gp.workspace,"nhdarea_tmp","30","8_BIT_UNSIGNED",SnapGrid)
    print gp.GetMessages()
...
try:
    ...
    gp.FeatureToRaster_conversion(nhd_wb_Layer, "dummy", wbtempraster, "30")
except:
    gp.CreateRasterDataset_management(gp.workspace,"nhdwb_tmp","30","8_BIT_UNSIGNED",SnapGrid)
    print gp.GetMessages()

# Process: Feature to Raster3 - NHD Flowline.  This is the first output
try:
    gp.FeatureToRaster_conversion(nhd_flow_Layer, "dummy", outraster1, "30")
except:
    print gp.GetMessages()

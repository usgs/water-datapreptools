...
# Created: 20100804
# ahrea@usgs.gov
# Usage: MakeCPG_percent <OutputWorkspace> <fdr> <fac> <ParameterGrid> <outgrid> 
# Description: 
# Makes a continuous parameter grid from a basin-percent type simple parameter grid. Also requires 
# a flowdirection and flowaccumulation grid. Works on parameters.
# ---------------------------------------------------------------------------
...
try: 
  ...
  gp.extent = fdr
  gp.cellSize = fdr
  gp.FlowAccumulation_sa(fdr, "tmpfacw", ParameterGrid, "INTEGER")

  # Process: Single Output Map Algebra...
  InExpression = "int((( tmpfacw + " + ParameterGrid + " ) / ( fac + 1.0 )) * 100 + 0.5)"
  GPMsg("",InExpression)
